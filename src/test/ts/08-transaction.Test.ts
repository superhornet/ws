...
/*describe("Testing the /api/transaction endpoint", () => {
    const lastSubstackIdentifier: string = "ee84918f-406d-4aa3-87ce-bd1e1c72b464";
    const adminSession: string = "06bd8763-8326-4ac4-b86e-418ac9ce3354";
    const companyFundsIdentifier: string = "83d13d18-3802-407f-b9b6-73f39b17e31d";
    const lastSession: string = "69dce0e3-e06c-48ed-86af-e5de2dc73fd3";

    test("Error 400 response when missing body of POST method", async () => {
        const handler = findRouteHandler(transactionRouter, 'post', '/transaction');
        assert.ok(handler, "Missing handler for transaction endpoint");
        const { req, res } = mockTransaction({ body: {} });
    ...
    test("Error 403 response when missing session of POST method", async () => {
        ...
        await handler(req.data, res, null);
        assert.equal(res.statusCode, 403);
        assert.equal(res.body.message, 'Session ID Required');
    });
    test("Add $5000 to Company Funds", async () => {
        const handler = findRouteHandler(transactionRouter, 'post', '/transaction');
        assert.ok(handler, "Missing handler for transaction endpoint");
        const { req, res } = mockTransaction({
            body: {
                ...
                amount: 5000,
                processor: TransactionProcessorType.INTERNAL,
                transactionType: TransactionItemType.INITIAL_FUND,
                toIdentifier: companyFundsIdentifier,
                fromIdentifier: ""
            }
        });
    ...
    test("Add $25 to Cotton Candy Substack", async () => {
        const handler = findRouteHandler(transactionRouter, 'post', '/transaction');
        assert.ok(handler, "Missing handler for transaction endpoint");
        const { req, res } = mockTransaction({
            body: {
                ...
                processor: TransactionProcessorType.GOOGLE,
                transactionType: TransactionItemType.INITIAL_FUND,
                toIdentifier: lastSubstackIdentifier,
                fromIdentifier: ""
            }
        });
    ...
    test("Add $96 to Flatscreen TV Substack", async () => {
        const handler = findRouteHandler(transactionRouter, 'post', '/transaction');
        assert.ok(handler, "Missing handler for transaction endpoint");
        const { req, res } = mockTransaction({
            body: {
                ...
                processor: TransactionProcessorType.GOOGLE,
                transactionType: TransactionItemType.INITIAL_FUND,
                toIdentifier: "307eeeef-c2eb-4e0e-965a-d7b55b153227",
                fromIdentifier: ""
            }
        });
    ...
    test("Fail by trying to transfer greater than available amount.", async () => {
        const handler = findRouteHandler(transactionRouter, 'post', '/transaction');
        assert.ok(handler, "Missing handler for transaction endpoint");
        const { req, res } = mockTransaction({
            body: {
                ...
                amount: 150,
                processor: TransactionProcessorType.INTERNAL,
                transactionType: TransactionItemType.DEBIT,
                toIdentifier: lastSubstackIdentifier,
                fromIdentifier: "307eeeef-c2eb-4e0e-965a-d7b55b153227"
            }
        });
        await handler(req.data, res, null);
        assert.equal(res.statusCode, 400);
    ...
    test("List Transactions by SubStackIdentifier", async () => {
        const handler = findRouteHandler(transactionRouter, 'get', '/transactions');
        assert.ok(handler, "Missing handler for transaction endpoint");
        const { req, res } = mockTransaction({
            body: {
    ...
    test("List Transactions by StackIdentifier", async () => {
        const handler = findRouteHandler(transactionRouter, 'get', '/transactions');
        assert.ok(handler, "Missing handler for transaction endpoint");
        const { req, res } = mockTransaction({
            body: {
                session: lastSession,
                stackID: "e3168ef7-7db3-4a9d-8cae-26ee59b024d7"
            }
        });
